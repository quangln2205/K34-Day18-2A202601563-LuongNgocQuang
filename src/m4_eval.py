...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    # Wrap in try/except — RAGAS needs OPENAI_API_KEY and Python 3.11+
    try:
...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    # For each EvalResult: compute avg of 4 metrics, find worst_metric
    results_with_avg = []
    for result in eval_results:
        avg_score = (result.faithfulness + result.answer_relevancy + 
    ...
    # Sort by average score ascending → take bottom_n
    sorted_results = sorted(results_with_avg, key=lambda x: x["score"])
